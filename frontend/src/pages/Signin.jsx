...
const Signin = () => {
  const [signinWith, setSigninWith] = useState("username");
  const [username, setUsername] = useState("");
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const navigate = useNavigate();
  const { authUser, signinFunction } = useAuthStore();

  const submitHandler = (e) => {
    e.preventDefault();
    signinFunction(username, email, password, signinWith);
  };
  ...
  return (
    <div className="flex items-center justify-center h-full w-screen border">
      <div className="h-[80%] w-[60%] backdrop-blur-2xl bg-slate-950 rounded-xl shadow-2xl flex gap-3">
        <div className="w-1/2 h-full flex gap-5 flex-col px-5 py-8">
          <div className="flex flex-col gap-2">
            <h1 className="text-4xl font-bold text-blue-400">Signin</h1>
          ...
          >
            ...
            <div className="flex flex-col gap-3 text-center">
              ...
              {signinWith === "username" ? (
                <p className="-mt-1 text-center cursor-pointer text-red-300" onClick={()=>setSigninWith("email")}>
                  Sign in with email
                </p>
              ) : (
                <p className="-mt-1 text-center cursor-pointer text-red-300" onClick={()=>setSigninWith("username")}>
